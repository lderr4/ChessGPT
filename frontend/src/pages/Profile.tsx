...
const Profile = () => {
  ...
  const handleUpdateProfile = async (e: React.FormEvent) => {
    ...
    try {
      await updateProfile({ chess_com_username: chessComUsername.trim() })
  ...
  useEffect(() => {
    ...
    const pollStatus = async () => {
      try {
        ...
        const { status, progress, imported_games, total_games, error_message } = response.data
        ...
        if (status === 'completed') {
          setMessage({
            type: 'success',
            text: `Import completed! ${imported_games} new games imported.`
          })
          setImportJobId(null)
          ...
          setMessage({
            type: 'error',
            text: error_message || 'Import failed. Please try again.'
          })
          setImportJobId(null)
          setIsImporting(false)
        }
  ...
  const handleImportGames = async () => {
    ...
    if (!trimmedUsername) {
      setMessage({ type: 'error', text: 'Please set your Chess.com username first' })
      return
    }
    ...
    try {
      ...
      setMessage({ 
        type: 'success', 
        text: `Importing ${dateRange}...` 
      })
    } catch (error: any) {
      setMessage({ 
        type: 'error', 
        text: error.response?.data?.detail || 'Failed to start game import' 
      })
      setIsImporting(false)
    }
  }
  ...
  return (
    <div className="max-w-2xl">
      ...
      <div className="bg-white rounded-xl shadow-sm p-6 mb-6">
        <h2 className="text-xl font-semibold text-gray-900 mb-4">Chess.com Integration</h2>
        <form onSubmit={handleUpdateProfile} className="space-y-4">
          ...
          >
            {isUpdating ? 'Updating...' : 'Update Chess.com Username'}
          </button>
        </form>
      </div>
      ...
      <div className="bg-white rounded-xl shadow-sm p-6">
        <h2 className="text-xl font-semibold text-gray-900 mb-4">Import Games</h2>
        <p className="text-gray-600 mb-4">
          Import your games from Chess.com. You can import all games or select a specific date range.
        </p>
        {user?.last_import_at && (
        ...
        <div className="mb-6 space-y-4">
          <div className="flex items-center gap-3">
            <label className="flex items-center gap-2 cursor-pointer">
              ...
              />
              <span className="text-gray-900 font-medium">Import all games</span>
            </label>
          </div>
          ...
          <div className="flex items-center gap-3">
            <label className="flex items-center gap-2 cursor-pointer">
              <input
                ...
                className="w-4 h-4 text-primary-600"
              />
              <span className="text-gray-900 font-medium">Import specific date range</span>
            </label>
          </div>
          ...
          <div className="mt-4">
            <div className="flex items-center justify-between mb-2">
              <span className="text-sm font-medium text-gray-700">
                {importStatus === 'processing' ? 'Importing games...' : 'Starting import...'}
              </span>
